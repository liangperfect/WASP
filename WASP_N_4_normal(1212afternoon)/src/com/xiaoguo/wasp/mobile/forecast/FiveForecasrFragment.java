...
public class FiveForecasrFragment extends Fragment {
	...
	private FiveDayForecastContentAdapter mAdapter;
	private TimeUtil timeUtil;
	private int index = 0;
	// 每次获取预报的条数
	private int rowRcount = 10;
	// 两个参数用于判断listView的长度是否超过屏幕
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		rootView = inflater.inflate(R.layout.five_forecast, container, false);
	...
	private void addListener() {
		...
		// 用于初始化的时候来判断ListView的高度是否超过了片屏幕
		...
		// System.out.println("各项参数的值--firstVisibleItem->>"
		// + firstVisibleItem + "  visibleItemCount--->>"
		// + visibleItemCount + "   ttotalItemCount---->>"
		// + totalItemCount);
		...
		// });
		//
		// View v = LayoutInflater.from(parentActivity).inflate(
		// R.layout.pulldown_footer, null);
		//
		// if (mVisibleItemCount == mTotalItemCount) {
		//
		// } else {
		// System.out.println("添加尾巴");
					...
					public void onRefresh(
						...
						commandBase.request(new TaskListener() {
							...
							public JSONObject requestData() {
								...
								try {
									data.put("type", "五天天气预报");
									data.put("rowCount", rowRcount);
									data.put("offset", index * rowRcount);
	...
	private void initView() {
		...
		mPullRefreshListView = (PullToRefreshListView) rootView
				.findViewById(R.id.fiveday_forecast_content_list);
		...
		commandBase.request(new TaskListener() {
			...
			public void messageUpdated(JSONObject msg) {
				...
				try {
					...
					for (int i = 0; i < list.length(); i++) {
						...
						map.put("time", timeUtil.getDateToString(publishDate
								.getLong(("time"))));
						// System.out.println("publishDate.getInt(time)-->>"
						// + timeUtil
						// .getDateToString(publishDate
			...
			public void finish() {
				// mPullRefreshListView.onRefreshComplete();
				// mAdapter.notifyDataSetChanged();
				mAdapter = new FiveDayForecastContentAdapter(parentActivity,
						content);
	...
	private class FiveDayForecastContentAdapter extends BaseAdapter {

		private ArrayList<HashMap<String, Object>> contentList;
		private Context context;

		public FiveDayForecastContentAdapter(Context context,
